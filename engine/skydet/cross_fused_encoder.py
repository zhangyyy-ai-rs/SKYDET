...
__all__ = ['CrossFusedEncoder']

class Conv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__()
        padding = k // 2 if p is None else p
        self.conv = nn.Conv2d(c1, c2, k, s, padding=padding, groups=g, bias=False)
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_weights = self.compress(x_compressed)
        spatial_weights = self.sigmoid(spatial_weights) # [B, 1, H, W]
        return spatial_weights.expand_as(x) # B, C, H, W] 

class ReciprocalGuidanceModule(nn.Module):
    def __init__(self, inc, ouc, reduction=16, spatial_kernel=7):
        super().__init__()
        ...
        if in_c0 != in_c1:
            self.adjust_conv = Conv(in_c0, in_c1, k=1)
            in_c0 = in_c1 
            
        self.spatial_gate_generator = SpatialGate(spatial_kernel)
        self.channel_gate_generator = ChannelGate(in_c1, reduction)

        final_in_channels = in_c1 * 2
        
        if final_in_channels != ouc:
            self.conv1x1 = Conv(final_in_channels, ouc, k=1)
        else:
    ...
    def forward(self, x):
        ...
        w_spatial = self.spatial_gate_generator(x0) # [B, C, H, W]
        w_channel = self.channel_gate_generator(x1) # [B, C, H, W]

        gated_x1 = x1 * w_spatial  
        gated_x0 = x0 * w_channel  

        out = torch.cat([x0 + gated_x1,  
                         x1 + gated_x0], 
                        dim=1)
...
@register()
class CrossFusedEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for _ in range(len(in_channels) - 1, 0, -1):       
            ...
            self.fpn_fea_fusion_blocks.append(  
                ReciprocalGuidanceModule([hidden_dim, hidden_dim], hidden_dim * 2)     
            )      
            self.fpn_blocks.append(
                RepNCSPELAN4(hidden_dim * 2, hidden_dim, hidden_dim * 2, round(expansion * hidden_dim // 2), round(3 * depth_mult), act=act) \
                if version == 'skydet' else CSPLayer(hidden_dim * 2, hidden_dim, round(3 * depth_mult), act=act, expansion=expansion, bottletype=VGGBlock)   
            )
        ...
        for _ in range(len(in_channels) - 1):    
            self.downsample_convs.append(  
                ...
                if version == 'skydet' else ConvNormLayer_fuse(hidden_dim, hidden_dim, 3, 2, act=act)   
            )
